...
def send_packet():
    ...
    try:
        ...
        print("Connected to server on port 9876")
        ...
        client_socket.sendto(b'\0', (host, port))  # End of message

        # Receive one UDP packet (server sends all at once)
        part, _ = client_socket.recvfrom(1024)
        ...
        lost_chars = ""
        for char in message:
            if (received_index < len(received_message) and char == received_message[received_index]):
                received_index += 1
            else:
                lost_chars += char
